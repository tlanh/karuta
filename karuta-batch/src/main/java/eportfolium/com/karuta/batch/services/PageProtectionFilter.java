...
import eportfolium.com.karuta.batch.pages.LogIn;
import eportfolium.com.karuta.batch.pages.infra.PageDenied;
 ...
 * {@link dulocaldansnosassiettes.web.annotation.ProtectedPage}. It examines
 * each {@link org.apache.tapestry5.services.Request} and redirects it to the
 * login page if the request is for a ProtectedPage and the user is not logged
 * in. If the page also has the {@link javax.annotation.security.RolesAllowed}
...
public class PageProtectionFilter implements ComponentRequestFilter {
	...
	private final String autoLoginStr = "false";
	// System.getProperty("dulocaldansnosassiettes.auto-login")
	...
	private final eportfolium.com.karuta.business.contract.SecurityManager securityFinderService;
	...
	public PageProtectionFilter(PageRenderLinkSource pageRenderLinkSource, ComponentSource componentSource,
			...
			eportfolium.com.karuta.business.contract.SecurityManager securityFinderService, UserManager userManager) {
		...
		this.logger = logger;
		this.securityFinderService = securityFinderService;
	...
	public AuthCheckResult checkAuthorityToPage(String requestedPageName) throws IOException {
		...
		if (rolesAllowed != null && !protectedPage) {
			throw new IllegalStateException(
					"Page \"" + requestedPageName + "\" is annotated with @RolesAllowed but not @ProtectedPage.");
		}

		// If page is public (ie. not protected), then everyone is authorised to it so
		// allow access
		...
		// If request is AJAX with no session, return an AJAX response that forces
		// reload of the page
	...
	private boolean isAuthenticated() throws IOException {
		...
		// - this facility is for development environment only. It avoids getting you
		// thrown out of the
	...
	private boolean isAuthorised(RolesAllowed rolesAllowed) throws IOException {
		...
		if (rolesAllowed == null) {
			...
			// Here we could check whether the user's role, or perhaps roles, include one of
			// the rolesAllowed.
			// Typically we'd cache the user's roles in the Visit.
			Visit v = sessionStateManager.getIfExists(Visit.class);
			if (v != null) {
				authorised = securityFinderService.isAdmin(v.getMyUserId());
			}
		}
		return authorised;
	}
	...
	/**
	 * Checks the value of system property dulocaldansnosassiettes.auto-login. If
	 * "true" then returns true; if "false" then return false; if not set then
	 * returns false.
	 */
	private boolean isAutoLoginOn() {
		...
		if (autoLoginStr == null) {
			...
			throw new IllegalStateException("System property dulocaldansnosassiettes.auto-login has been set to \""
					+ autoLoginStr
	...
	/**
	 * Automatically logs you in as the given user. Intended for use in development
	 * environment only.
	 */
	private void autoLogin(Integer customerId) {

		// Lazy-load the business services locator because it is only needed for
		// auto-login

		try {
			Credential customer = userManager.getUser(customerId);

			Visit visit = new Visit(customer);
			logger.info(customer.getEmail() + " has been auto-logged-in.");
