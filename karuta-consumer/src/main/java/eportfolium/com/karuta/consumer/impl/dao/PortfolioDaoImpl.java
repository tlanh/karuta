...
public class PortfolioDaoImpl extends AbstractDaoImpl<Portfolio> implements PortfolioDao {
	...
	public String getPortfolioUuidFromNode(String nodeUuid) {
		return getPortfolioUuidFromNode(nodeUuid);
	}
...
//			sql += " WHERE gu.id.credential.id = : userId;
	...
	public String getPortfolioUuidFromNodeCode(String portfolioCode) {
	...
	public String getPortfolioModelUuid(String portfolioUuid) {
		String modelID = null;
	...
	public boolean changePortfolioOwner(String portfolioUuid, Long ownerId) {
		...
		try {
			...
			String nodeUuid = p.getRootNode();
			Node rootNode = nodeDao.findById(nodeUuid);
	...
	private Portfolio add(Node rootNode, Credential credential, Portfolio portfolio) throws BusinessException {
		...
		// sauvegarde ne doit pas en créer un nouveau.
		if (!Long.valueOf(cr.getId()).equals(credential.getId())) {
	...
	public boolean hasSharedNodes(String portfolioUuid) {
		...
		for (Node n : nodes) {
			String sharedNode = n.getSharedNodeUuid();
			if (sharedNode != null) {
				result = true;
				break;
			}
